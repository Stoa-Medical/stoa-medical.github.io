<script lang="ts">
  ...
  // Define component props using Svelte 5 runes
  const {
    name = "medical", // which brand glyph to render
    width = 250,
    height = 343,
    variant = undefined, // explicit colour override
  } = $props<{
    name?: "medical" | "portico"; // extend with more brand icons as needed
    width?: number;
    height?: number;
    variant?: "light" | "dark" | "tan" | "grey" | undefined;
  }>();

  // Reactive colour based on theme or explicit variant
  let currentColor = $state(getColorForTheme());

  // Update colour when system theme changes (unless an explicit variant is set)
  $effect(() => {
    if (!variant) {
  ...
  // React to explicit variant changes
  $effect(() => {
    if (variant === "light") {
      currentColor = "#FDFCFB"; // white for dark backgrounds
    } else if (variant === "dark") {
      currentColor = "#262431"; // ink for light backgrounds
    } else if (variant === "tan") {
      currentColor = "#F8F2E3";
  ...
  function getColorForTheme(): string {
    return $theme === "dark" ? "#FDFCFB" : "#262431";
  }
</script>

{#if name === "medical"}
  <!-- Stoa Medical logomark: sigma + snake -->
  <svg
    {width}
    {height}
    viewBox="0 0 250 343"
    ...
    aria-label="Stoa Medical logomark"
  >
    <path
      fill-rule="evenodd"
      clip-rule="evenodd"
...
{:else if name === "portico"}
  <!-- Placeholder for future Portico icon -->
  <svg
    {width}
    {height}
    viewBox="0 0 24 24"
    xmlns="http://www.w3.org/2000/svg"
    aria-label="Stoa Portico icon placeholder"
  >
